  ...
  <section class="explain-block">
    ...
    <template v-for="(seg, i) in segments" :key="i">
        ...
        <template v-for="(item, idx) in seg.items" :key="idx">
          <li>
            ...
            <ul v-if="item.children">
              <li v-for="(child, ci) in item.children" :key="ci"><InlineText :text="child.text" /></li>
            </ul>
          </li>
        </template>
      </ul>
      ...
      <table v-else-if="seg.type === 'table'" class="block-table">
        <thead>
          <tr>
            <th v-for="(h, j) in seg.headers" :key="j"><InlineText :text="h" /></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(row, ri) in seg.rows" :key="ri">
            <td v-for="(cell, ci) in row" :key="ci"><InlineText :text="cell" /></td>
          </tr>
        </tbody>
      </table>
      <blockquote v-else-if="isTextContentSegment(seg) && isBlockquoteText(seg.text)" class="md-blockquote">
        <p class="block-text">
