...
function getLangExtension(tab: Tab) {
  switch (tab) {
    case "html":
      return [html()];
    case "css":
      return [css(), closeBrackets()];
    case "js":
      return [javascript({ typescript: false }), closeBrackets()];
  }
}
...
async function doFormat() {
  ...
  try {
    ...
    const parserMap: Record<Tab, string> = {
      html: "html",
      css: "css",
      js: "babel",
    };
    const pluginMap: Record<Tab, () => Promise<unknown>> = {
      ...
      js: () =>
        import("prettier/plugins/babel").then(async (b) => {
...
<template>
  <div class="editor-panel">
    <div class="editor-tabs">
      ...
      >
        <span :class="['live-label', { 'live-label--on': livePreview }]"
          >实时</span
        >
