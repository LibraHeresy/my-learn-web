...
export const projects: Project[] = [
  // ================================================================
  // 项目1：音乐家名片页 — 纯 HTML/CSS，sandbox 模式
  // ================================================================
  {
    id: "music-showcase",
    trackId: "projects",
    order: 1,
    title: "音乐家名片页",
    subtitle: "用 HTML 和 CSS 制作你的第一张音乐家数字名片",
    icon: "🎵",
    mode: "sandbox",
    musicAnalogy:
      '就像第一次登上舞台演奏一首简单的练习曲——不需要复杂的技巧，只需要把每一个音符（标签）和表情记号（样式）准确地呈现出来。完成后，你就拥有了第一张属于自己的"音乐数字名片"。',
    prerequisiteTrackIds: ["fundamentals"],
    listenTo:
      "巴赫《G大调小步舞曲》— 简单、优雅、结构清晰，正如你即将写下的第一张网页。",
    steps: [
      {
        title: "搭建名片结构",
        content:
        ...
        task: '用 HTML 搭出一张音乐家名片，包含名字（h1）、简介（p）、代表作列表（ul > li）、照片（img）。全部包在一个 `<div class="composer-card">` 里。',
        purpose: "这是项目的起点——把你在第一章学到的 HTML 标签综合运用。`h1`、`p`、`ul`、`li`、`img`、`div` 单独用都很简单，但组合起来才构成一张完整的卡片。这一步练习的是信息架构：先想清楚名片上需要什么信息，再选对应的标签。",
        expectedResult: "预览区中出现了一张信息卡片——有标题、段落、列表和图片。虽然还没有样式，但所有内容都清晰可读地排列在卡片中。这是你名片的地基。",
        hint: "图片链接可以从维基百科获取。如果维基百科图片加载失败（常见于热链接限制），可以用 `https://placehold.co/400x300/E8D5B7/8B2E2E?text=Composer` 作为占位图。另外，给你的名片加一个超链接 `<a href=\"...\">` 指向音乐家的维基百科页面或作品试听链接，让卡片更有交互感！",
        starterCode: {
          html: '<div class="composer-card">\n  <img src="https://upload.wikimedia.org/wikipedia/commons/6/6f/Beethoven.jpg" alt="贝多芬">\n  <h1>路德维希·凡·贝多芬</h1>\n  <p>德国作曲家、钢琴家。他是古典音乐史上最伟大的作曲家之一，也是浪漫主义音乐的先驱。</p>\n  <h3>代表作</h3>\n  <ul>\n    <li>第五交响曲"命运"</li>\n    <li>第九交响曲"合唱"</li>\n    <li>月光奏鸣曲</li>\n    <li>致爱丽丝</li>\n  </ul>\n</div>',
          css: "",
          js: "",
        },
      },
      {
        ...
        task: "在 CSS 编辑区美化你的名片：给卡片设置背景色、圆角、阴影、内边距和最大宽度。给图片设置合适的圆角。",
        purpose: "这一步让你理解 CSS 的核心角色——视觉表现层。HTML 决定有什么（名字、图片、列表），CSS 决定长什么样（颜色、圆角、阴影）。把内容和表现分开，是网页设计最基本的原则。以后你维护一个网站时，改颜色只需要改 CSS，不用动 HTML。",
        expectedResult: "添加 CSS 后，名片从纯文字堆叠变成一张有背景色、有圆角、有阴影的精致卡片。图片也有了圆角。预想效果：像一张优雅的音乐家名片，而不是一份纯文本文档。",
        hint: '试试 `box-shadow: 0 4px 16px rgba(0, 0, 0, 0.12)` — 这是经典的"卡片悬浮"阴影。背景色建议用暖色系，如 `#FFFAF2` 或 `#FDF8F0`。',
        starterCode: {
          html: '<div class="composer-card">\n  <img src="https://upload.wikimedia.org/wikipedia/commons/6/6f/Beethoven.jpg" alt="贝多芬">\n  <h1>路德维希·凡·贝多芬</h1>\n  <p>德国作曲家、钢琴家。他是古典音乐史上最伟大的作曲家之一，也是浪漫主义音乐的先驱。</p>\n  <h3>代表作</h3>\n  <ul>\n    <li>第五交响曲"命运"</li>\n    <li>第九交响曲"合唱"</li>\n    <li>月光奏鸣曲</li>\n    <li>致爱丽丝</li>\n  </ul>\n</div>',
      ...
      },
      {
        ...
        task: "调整名片排版细节：给代表作列表添加背景色和圆角，给列表项之间加点间距（line-height 或 margin）。尝试自定义列表标记符号。",
        purpose: "这一步教你排版细节的打磨——CSS 不只是能加颜色和阴影，还能精细控制每个元素内部的间距、对齐和装饰。`::marker` 选择器让你自定义列表标记，这是 CSS 伪元素的典型应用。排版的核心是：让重要信息突出，让阅读体验舒适。",
        expectedResult: "代表作区域变成了一个有独立背景色和圆角的小盒子，列表项之间间距更舒适了。如果设置了 `::marker`，列表的圆点变成了你指定的符号。整个名片看起来更加精致。",
        hint: '如果想改变列表标记，除了 `::marker` 还可以用 `list-style-type: "🎵 "`（注意 Firefox 支持有限）。兼容性最好的做法是用 `list-style: none` 然后在每个 `<li>` 前面手动加表情。',
        starterCode: {
      ...
      },
      {
        ...
        task: "把名片改成你喜欢的音乐家。调整颜色、字体、布局，直到你满意为止。尝试至少修改 5 处不同的 CSS 属性。小提示：可以试试把外层的 `<div class=\"composer-card\">` 改成 `<article class=\"composer-card\">`（语义化标签），并添加一个指向该音乐家更多信息的链接。",
        purpose: "这是自由创作阶段——前几步是在学规则，这一步是让你突破规则。把贝多芬换成你真正喜欢的音乐家，配色方案改成你的审美。真正的学习不是照着做，而是用自己的判断力去调整和创造。完成的这张名片是你在代码乐章的第一个独立作品。",
        expectedResult: "名片完全变成了你的个人作品——音乐家换成了你选的、配色换成了你的风格、可能还多了新加的链接或内容。这张名片是你从学习到创造的分界线：你不再是一个教程的跟随者，而是一个能独立决定页面效果的设计者。",
        hint: '想知道某种颜色好不好看？试试在浏览器中打开 `DevTools`（`F12`），然后点击元素检查器，直接在 `Styles` 面板中修改 CSS 值——页面会实时更新。这是设计师和开发者最常用的"调色板"。',
        starterCode: {
          html: '<div class="composer-card">\n  <img src="https://upload.wikimedia.org/wikipedia/commons/6/6f/Beethoven.jpg" alt="贝多芬">\n  <h1>路德维希·凡·贝多芬</h1>\n  <p>德国作曲家、钢琴家。他是古典音乐史上最伟大的作曲家之一，也是浪漫主义音乐的先驱。</p>\n  <div class="works-section">\n    <h3>🎼 代表作</h3>\n    <ul>\n      <li>第五交响曲"命运"</li>\n      <li>第九交响曲"合唱"</li>\n      <li>月光奏鸣曲</li>\n      <li>致爱丽丝</li>\n    </ul>\n  </div>\n</div>',
          css: '.composer-card {\n  max-width: 400px;\n  margin: 20px auto;\n  padding: 24px;\n  background: #FFFDF7;\n  border-radius: 12px;\n  box-shadow: 0 4px 16px rgba(0, 0, 0, 0.12);\n  text-align: center;\n  font-family: "Noto Serif SC", "STSong", serif;\n}\n.composer-card img {\n  width: 100%;\n  border-radius: 8px;\n  margin-bottom: 16px;\n}\n.composer-card h1 {\n  font-size: 1.5rem;\n  color: #3D2B1F;\n  margin: 0 0 8px 0;\n}\n.composer-card > p {\n  font-size: 0.9rem;\n  color: #6B5A4E;\n  line-height: 1.6;\n  margin: 0 0 16px 0;\n}\n.works-section {\n  background: #FBF7F0;\n  border-radius: 8px;\n  padding: 16px;\n  text-align: left;\n}\n.works-section h3 {\n  font-size: 0.95rem;\n  color: #8B2E2E;\n  margin: 0 0 8px 0;\n}\n.works-section ul {\n  color: #6B5A4E;\n  font-size: 0.9rem;\n  line-height: 2;\n  padding-left: 20px;\n  margin: 0;\n}',
          js: "",
  ...
  // ================================================================
  {
    ...
    mode: "local" as const,
    musicAnalogy:
      "就像亲手抄写一份乐谱——每一个音符（标签）、每一个表情记号（样式）、每一个演奏指令（脚本）都由你亲手写下。完成后，你就拥有了一份独一无二的手稿。",
    prerequisiteTrackIds: ["fundamentals", "framework"],
    listenTo:
      "巴赫《无伴奏大提琴组曲》— 单一乐器展现完整世界，正如纯 HTML/CSS/JS 不依赖任何框架",
    steps: [
      {
        title: "环境准备 — 打开你的 VS Code",
        content:
        ...
        task: "安装 `VS Code`，创建 `music-collection` 项目文件夹，新建 `index.html`、`style.css`、`script.js` 三个文件，并正确关联它们。用浏览器打开 `index.html` 确认基本结构能正常显示。",
        purpose: "从在线编辑器切换到本地 IDE 是重要的一步。VS Code + 文件系统 + 浏览器 = 真正的开发环境。这一步让你建立起本地开发的基本工作流，后面所有的项目都基于此。",
        expectedResult: "在 VS Code 中看到三个文件，用浏览器打开 index.html 能看到基本页面结构。F12 控制台没有报错（红色错误）。",
        hint: "`VS Code` 的 `! + Tab` 缩写叫 `Emmet`。如果没生效，检查文件是否以 `.html` 结尾。`Live Server` 插件在 `VS Code` 的扩展商店（`Ctrl+Shift+X`）搜索 `Live Server` 安装，它会在保存文件时自动刷新浏览器。",
      },
      {
        title: "搭建收藏卡片",
        content:
        ...
        purpose: "从零搭建一张卡片——复习第一章学过的标签（`h2`、`p`、`img`、`span`），并用 `div` 包裹成一张完整的卡片。后面的 CSS 和 JS 都基于这张卡片的结构。",
        expectedResult: "预览区中出现一张音乐卡片，包含图片、曲名、作曲家和风格标签。虽然还没有样式，但信息结构清晰完整。",
      ...
      },
      {
        ...
        purpose: "这一步让你感受 CSS 的魔力——同样的 HTML 结构，加上几行样式就焕然一新。圆角让卡片柔和不生硬、阴影让卡片有了层次感、配色让卡片有了温度。就像给一段旋律加上和声与配器，同样的音符，完全不同的听感。CSS 就是网页的'配器师'。",
        expectedResult: "预览区中的卡片从纯文字堆叠变成了一张精致的卡片——有了圆角、阴影和背景色，标签也变成了小巧的圆角徽章。页面开始有了一种'设计感'，不再像一份纯文本文档。",
      ...
      },
      {
        ...
        purpose: "一张卡片是独奏，三张卡片就是室内乐。这一步教你用 CSS Grid 把多张卡片排列整齐，更重要的是理解响应式设计的核心思想——`auto-fill` 和 `minmax` 让卡片自动适应屏幕宽度，一行放不下就自动换行，完全不需要媒体查询。这是现代 CSS 布局最重要的技能之一。",
        expectedResult: "预览区中出现了 3 张排列整齐的卡片，屏幕宽时并排显示，屏幕窄时自动变成单列。CSS Grid 替你完成了所有排列计算，卡片间距均匀，整体布局美观。",
      ...
      },
      {
        ...
        purpose: "这是你的 JavaScript 第一次真正'操控'页面——不是简单地显示静态内容，而是根据用户的操作动态改变页面。筛选功能背后的核心模式（选中所有元素→遍历判断→修改显示/隐藏）几乎会出现在每一个前端交互中。这个循环是 DOM 操作的基本功，就像音阶练习之于演奏家。",
        expectedResult: "页面上方有一排筛选按钮（全部、巴洛克、古典等）。点击任意按钮，只显示对应风格的音乐卡片，其余卡片消失。'全部'按钮恢复所有卡片。当前选中的按钮有高亮样式，用户一眼就知道在看哪个分类。",
        ...
        starterCode: {
          ...
          js: '// 筛选功能骨架 — 请完成下面的代码\nconst filterBtns = document.querySelectorAll(".filter-btn");\nconst cards = document.querySelectorAll(".music-card");\nfilterBtns.forEach(function(btn) {\n  btn.addEventListener("click", function() {\n    // 1. 移除所有按钮的 active class\n    filterBtns.forEach(b => b.classList.remove("active"));\n    // 2. 给当前点击的按钮加上 active class\n    this.classList.add("active");\n    // 3. 获取当前按钮的 data-filter 值\n    const filter = this.dataset.filter;\n    // 4. 遍历所有卡片，根据 data-period 判断显示或隐藏\n    //    提示：card.dataset.period 可以获取卡片的时期\n  });\n});',
        },
      },
      {
        title: "添加收藏表单",
        ...
        purpose: "从'显示静态数据'到'动态创建内容'是一大步。你学会了用 JavaScript 生成 HTML（`createElement` + `appendChild`），让页面不再是写死的，而是可以不断扩展的。这是前端交互的起点——用户不只是被动的观众，而是能主动丰富收藏的参与者。每一次添加，都是你的代码在'创作'。",
        expectedResult: "在表单中填写曲名、作曲家和时期后点击添加，一张全新的卡片被动态生成并插入到网格末尾。新卡片和预设卡片的样式完全一致，筛选功能对它同样有效。添加后表单自动清空，可以继续添加下一首曲子。",
        ...
        starterCode: {
          ...
          js: '// 添加收藏功能骨架 — 请完成下面的代码\nconst addBtn = document.querySelector("#addBtn");\nconst cardGrid = document.querySelector("#cardGrid");\naddBtn.addEventListener("click", function() {\n  // 1. 读取输入框的值\n  const title = document.querySelector("#titleInput").value.trim();\n  const composer = document.querySelector("#composerInput").value.trim();\n  const image = document.querySelector("#imageInput").value.trim();\n  const period = document.querySelector("#periodInput").value;\n  \n  // 2. 验证必填字段\n  if (!title || !composer || !period) return;\n  \n  // 3. 用 document.createElement 创建新的 .music-card 元素\n  // 4. 设置卡片内容（img, h2, p, span.tag）\n  // 5. 把新卡片 appendChild 到 cardGrid 中\n  // 6. 清空输入框\n});',
        },
      },
    ],
  },
  {
    ...
    mode: "local" as const,
    musicAnalogy:
      "打开窗户，聆听世界的声音——你的应用第一次与互联网上的真实数据对话。就像从独奏练习室走到音乐图书馆，你可以搜索任何一位作曲家、任何一首曲子。",
    prerequisiteTrackIds: ["fundamentals", "framework"],
    listenTo:
      "雷斯庇基《罗马的松树》— 作曲家通过音乐描绘远方的风景，正如你通过 API 获取远方的数据。一个请求，带回整个世界的音乐。",
    steps: [
      {
        ...
        purpose: "API 是前端和互联网的接口——就像一根电话线连接了两个房间。这一步不急着写代码，而是先理解你要对话的对象：iTunes Search API 返回什么样的数据？每个字段代表什么？只有先摸清'对方说什么'，后面写请求代码时才能精准地提取需要的信息。探索 API 是每个前端开发者拿到新接口后的第一件事。",
        expectedResult: "浏览器中显示了一串 JSON 数据，你能认出 `trackName`（歌名）、`artistName`（艺术家）、`artworkUrl100`（封面图）、`previewUrl`（试听链接）等关键字段。修改 URL 参数（如 `term=mozart`）后，返回的结果也跟着变化——你理解了 API 的请求-响应模式。",
        hint: "JSON 看起来像 JS 对象，但它是字符串格式。`JSON.parse()` 可以将 JSON 字符串转为 JS 对象。字段名用英文是因为 API 是国际化的。\n> ⚠️ **关于 CORS**：浏览器出于安全考虑，禁止网页随意请求其他域名的数据——这叫同源策略。iTunes Search API 之所以可以跨域访问，是因为它设置了正确的 CORS 响应头（`Access-Control-Allow-Origin: *`），允许任何网站调用。如果直接用 `file://` 协议打开 HTML 文件，某些浏览器可能阻止 fetch 请求——建议使用 Live Server 或 `npx serve` 来本地预览。",
      ...
      },
      {
        ...
        purpose: "这是 fetch 的实战首秀——异步请求、await 等待、JSON 解析、DOM 渲染，四个环节串成一条完整的数据链路。理解这条链路上的每一步，以后调用任何 API（天气、地图、翻译）都是同样的模式。`encodeURIComponent` 处理中文参数这个细节，也是实际开发中必知必会的技巧。",
        expectedResult: "在输入框中输入'贝多芬'，点击搜索按钮，页面上出现与贝多芬相关的音乐列表——每条包含封面图、歌名和艺术家名。数据从互联网实时获取，不是写死在代码里的。Network 面板中能看到 API 请求和 JSON 响应。",
      ...
      },
      {
        ...
        purpose: "好的用户体验不是功能多，而是细节到位。实时搜索让用户省去点击按钮的动作，防抖让 API 不会被每次按键都触发一次请求——就像乐团指挥不会为每一个错音停下来，而是等一段结束后再做出调整。`clearTimeout` + `setTimeout` 这个经典组合，你会在无数前端场景中反复遇到。",
        expectedResult: "输入框中每打一个字，搜索结果都在 400ms 停顿后自动更新。快速连续输入'贝多芬第五交响曲'时，Network 面板中只看到一次 API 请求——防抖生效了，避免了对服务器的无效轰炸。",
      ...
      },
      {
        ...
        purpose: "前端工程师和教程练习者的分界线就在这里——专业应用永远不会让用户面对'什么都不发生'的空白页面。idle、loading、success、empty、error 这五种状态覆盖了用户可能遇到的每一种情况。这一步培养的是'边界思维'：不仅要处理风和日丽的正常流程，还要优雅地处理狂风暴雨的异常场景。",
        expectedResult: "搜索器在各种情况下都有对应的界面反馈：初始状态引导用户输入、搜索中显示加载动画或文字、有结果时正常展示列表、无结果时友好提示'没找到相关音乐'、断网时显示错误提示。用户在任何情况下都不会感到困惑。",
      ...
      },
      {
        ...
        purpose: "做完项目后的回顾比做项目本身更重要——把零散的技能点串成完整的知识图谱。async/await、fetch、try/catch、防抖、DOM 操作、状态管理——它们不是孤立的技巧，而是构成一个完整前端应用的六个支柱。理解它们之间的关系，比记住它们的语法更有用。这也是你从'会写'到'真懂'的转变。",
        expectedResult: "搜索器经过完整测试，五种状态（idle、loading、success、empty、error）切换流畅自然。你清楚地知道这个项目中每一行代码的作用，能够向别人解释它是怎么工作的。从请求发出到页面渲染，整条数据链路如交响乐般在你脑中清晰奏响。",
  ...
  },
  {
    ...
    mode: "local" as const,
    musicAnalogy:
      "独奏者已经熟练了曲子——现在是时候加入乐团了。Vue 的组件就像乐团中的不同声部，各司其职又和谐统一。",
    prerequisiteTrackIds: ["fundamentals", "framework", "engineering"],
    listenTo:
      "拉威尔《波莱罗舞曲》— 同一段旋律在不同乐器组之间传递、叠加、变奏。Vue 的组件就像乐团中的不同声部，各司其职又和谐统一。",
    steps: [
      {
        title: "创建 Vue 项目",
        content:
        ...
        purpose: "这一步把你在'登台篇'学到的 Vite 脚手架技能付诸实践。Vite 生成的空白项目就像一个空白总谱——目录结构、配置文件、入口文件各就各位，等你来填充内容。理解项目的骨架结构很重要，因为后面所有开发都建立在这个基础上。",
        expectedResult: "终端中 Vite 脚手架创建成功，`npm run dev` 后浏览器显示 Vue 默认欢迎页面。VS Code 中能看到清晰的项目目录结构：`index.html`、`src/App.vue`、`src/main.js`、`src/components/`，一切都井井有条。",
      ...
      },
      {
        ...
        purpose: "写代码之前先设计——这是工程师和'边写边想'的初学者的分水岭。组件树设计让你在动手之前就想清楚：数据放在哪？谁管状态？谁发事件？就像乐团的分谱——每个声部拿到的是属于自己的那一份，指挥（App.vue）掌握总谱。花 10 分钟画一张图，能省 1 小时的排错时间。",
        expectedResult: "你有一张清晰的组件树图：App.vue 在最顶层（数据和状态的主人），FilterBar、MusicCard、AddForm 三个子组件各司其职、互不干扰。`src/components/` 下有三个 `.vue` 文件准备就绪。",
      ...
      },
      {
        ...
        purpose: "MusicCard 是你写的第一个真正的 Vue 组件——它接收数据（Props）展示内容，响应用户操作（Emits）通知父组件，但从不修改传入的数据（单向数据流）。这个组件就像乐团中的一位演奏家：拿到分谱（Props）就演奏，需要调整时举手示意指挥（Emits），但从来不擅自改总谱。",
        expectedResult: "MusicCard 组件能独立展示一张音乐卡片：封面图、曲名、作曲家、时期标签。鼠标悬停时圆角删除按钮淡入显示，点击收藏按钮在 🤍 和 ❤ 之间切换。所有交互通过 emit 通知父组件，子组件内部数据不变。",
      ...
      },
      {
        ...
        purpose: "FilterBar 体现了组件化的精髓——封装变化。这个组件不关心外面是谁在用、数据怎么来、结果怎么处理，只管展示一排按钮和处理点击。它的'接口'极其简洁：接收 periods 列表和 active 当前值，发出 update:active 事件。以后换样式、改交互，都只需改这一个文件，对全局零影响。",
        expectedResult: "一排美观的金色边框筛选按钮显示在页面上，当前选中的按钮有金色填充高亮样式。点击任意按钮，高亮即时切换，父组件通过 `v-model:active` 自动收到筛选事件，卡片列表随之变化。",
      ...
      },
      {
        ...
        purpose: "AddForm 展示了 Vue 的 `v-model` 有多么简洁——对比 v1 中需要手动 `querySelector` + `addEventListener` + 拼 HTML 的繁琐流程，现在只需 `v-model` 双向绑定 + `emit` 发送对象。代码量减少 70%，可读性提升 200%。这就是框架的核心价值：把重复劳动自动化，让你专注于业务逻辑。",
        expectedResult: "表单包含四个输入字段，必填项有验证提示。填写曲名、作曲家等信息后点击提交，新曲目被添加到收藏列表中，表单自动清空。如果必填字段为空，表单下方显示红色错误提示，数据不会被提交。",
      ...
      },
      {
        ...
        purpose: "这是 Vue 组件化开发的高潮——所有子组件就位，App.vue 作为指挥将它们协调起来。`computed` 自动计算筛选结果、`v-for` 渲染卡片列表、事件处理函数修改数据。你看到的是数据在组件树中单向流动：子组件 emit 事件 → App.vue 修改 ref → computed 重新计算 → 模板自动更新。理解了这条流动路径，就理解了 Vue 的响应式灵魂。",
        expectedResult: "页面完整呈现：筛选栏、添加表单、统计信息（'筛选出 N 首中的 M 首'）、卡片网格，所有功能协调运转。筛选按钮能过滤卡片，添加表单能新增曲目，每张卡片上的收藏和删除按钮都能正常工作。所有组件如默契的室内乐团般和谐运转。",
      ...
      },
      {
        ...
        purpose: "数据持久化让应用有了'记忆'——刷新页面后数据不丢失。`watch` 比手动调用 `localStorage.setItem` 优雅得多：设置一次监听，之后所有数据变更都自动存档，永远不会漏掉。`{ deep: true }` 选项确保数组内部的增删改都能被检测到。这是声明式编程的又一次胜利：你只需描述'监控这个数据'，不用在每个修改点手动保存。",
        expectedResult: "刷新页面后，所有收藏的曲目都还在——添加过的、修改过收藏状态的、删除的，都如实保留。打开浏览器 DevTools 的 Application → Local Storage，能看到 `music-collection-vue` 键中保存的 JSON 数组，与 `pieces` 的数据完全同步。",
      ...
      },
      {
        ...
        purpose: "比较是最好的学习方式。v1 是命令式——你告诉浏览器'怎么做'（找到这个元素、改它的样式、创建那个元素），v2 是声明式——你描述'应该是什么样'（筛选结果是这个数组、卡片用这条数据渲染）。从命令式到声明式的转变，是你从前端初学者迈向专业开发者的关键一步。理解了这一点，就理解了所有现代前端框架存在的意义。",
        expectedResult: "v1 和 v2 功能完全相同，但 v2 的代码更短、更清晰、更容易修改。你能自信地向别人解释：为什么 Vue 让开发更高效，以及 Props/Emits/Computed/Watch 各自解决了什么问题。这种对比理解比单独学习任何一个框架都更有价值。",
  ...
  },
  {
    ...
    mode: "local" as const,
    musicAnalogy:
      "从排练厅走上音乐厅的舞台——Git 版本控制是你的排练记录，CI/CD 是你的演出流程，云端部署就是座无虚席的音乐厅。",
    prerequisiteTrackIds: ["fundamentals", "framework", "engineering"],
    listenTo:
      '贝多芬《第九交响曲"合唱"》第四乐章 — 从器乐到人声，从个体到群体，所有力量汇聚成《欢乐颂》的洪流。Git、GitHub、CI/CD 就是你的乐团、音乐厅和全球巡演。',
    steps: [
      {
        title: "Git 初始化与首次提交",
        content:
        ...
        purpose: "Git 是程序员的'乐谱存档系统'——每一次 commit 都是一次快照，记录了你项目在某一刻的完整状态。`.gitignore` 就像告诉档案管理员'node_modules 不用归档，别人能自己下载'。养成频繁提交的习惯，就像排练时录下每一次演奏，方便随时回听和改进。",
        expectedResult: "`git log` 显示第一次提交记录，带有一个哈希值和你写的提交信息。`git status` 显示工作区干净（`nothing to commit, working tree clean`），所有源码文件都已追踪。",
      ...
      },
      {
        ...
        purpose: "分支是 Git 最强大的功能——它让你在'草稿本'上随意实验，不影响已经稳定的主版本。就像作曲家在同一首交响曲上尝试不同的配器版本，试好了合并到正式总谱，试不好直接丢弃。没有分支的 Git 就像没有排练室的乐团——只能在音乐厅直接冒险演出。",
        expectedResult: "在 `feature/favorites` 分支上开发了收藏夹筛选功能，合并回 main 后功能完整可用。`git log --oneline --graph` 能看到清晰的分支分叉和合并历史，就像乐谱上的声部标记一样明确。",
      ...
      },
      {
        ...
        purpose: "代码只有上传到 GitHub，才算真正'发表'——就像乐谱从个人抽屉里拿出来，放在了音乐图书馆的公开书架上。GitHub 不只是代码存储，更是你的技术名片：别人能看到你的项目、你的代码风格、你的 commit 习惯。这是你从'自己写着玩'到'参与技术社区'的第一步。",
        expectedResult: "GitHub 仓库页面显示了你的完整项目文件，README 正常渲染。`git remote -v` 显示 origin 指向正确的 GitHub 地址。之后每次 `git push` 都能把本地修改同步到云端。",
      ...
      },
      {
        ...
        purpose: "开发版本和生产版本的关系，就像排练录音和正式唱片——前者有注释、有调试信息、方便修改，后者经过压缩和优化，是给最终听众的成品。理解这个区别很重要：你写的 Vue 单文件组件不会直接发给浏览器，而是经过 Vite 编译成浏览器原生能运行的 HTML/CSS/JS。",
        expectedResult: "`dist/` 文件夹中包含了压缩后的 HTML、JS 和 CSS 文件——变量名被缩短、空格被删除、注释被移除。用 Vite 预览命令打开后，功能与开发模式完全一致，但文件体积更小、加载更快。这是上线前的最后质检。",
      ...
      },
      {
        ...
        purpose: "部署是将你的作品从本地推向世界的关键一步。GitHub Pages 是免费、稳定的静态托管方案——你只需要把构建好的文件推到指定分支，它就帮你服务到公网。这一步让你体会到从前端开发的完整闭环：写代码 → 构建 → 部署 → 别人通过 URL 访问。这个流程是所有前端项目的标配。",
        expectedResult: "浏览器中访问 `https://你的用户名.github.io/music-collection/`，你的音乐收藏库完整呈现在公网上。任何人都可以通过这个链接看到你的项目——添加、筛选、收藏、删除功能全部正常工作。你创作了一个真正可以分享的作品。",
      ...
      },
      {
        ...
        purpose: "手动部署容易忘、容易出错——CI/CD 用自动化替代了重复劳动。每次 git push 后，GitHub Actions 自动帮你构建和部署，就像请了一位舞台助理帮你完成每次演出的灯光和布景。这种'提交即部署'的流程是专业团队的标准实践，你从现在开始就走在正确的工程化道路上。",
        expectedResult: "push 代码到 GitHub 后，Actions 标签页中出现一条正在运行的 workflow（黄色旋转图标 → 绿色对勾）。运行完成后，GitHub Pages 上的网站自动更新。修改页面 → git push → 等一两分钟 → 线上已更新，全程无需手动运行任何部署命令。",
      ...
      },
      {
        ...
        purpose: "回头看走过的路，才知道走了多远。从 `<h1>Hello World</h1>` 到 Vue 组件化开发、Git 版本管理、GitHub Pages 部署、CI/CD 自动化——这不是几门独立课程的堆砌，而是一条完整的职业能力成长线。分享你的作品不仅是对自己努力的肯定，更是打开参与技术社区的大门。每个音乐家都是从第一个音符开始的，你从第一行代码走到了自动化部署——这不是终点，而是序曲的结束、第一乐章的起点。",
        expectedResult: "GitHub 上的音乐收藏库正常运行，所有功能（添加、筛选、收藏切换、删除、刷新保留数据、收藏夹筛选）在线上也能正常工作。你把链接分享给了至少一个人，完成了从学习者到创造者的蜕变。这是你的'毕业音乐会'——掌声为你而响。",
