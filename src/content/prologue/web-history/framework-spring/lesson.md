...
为此，Google 在丹麦招来了一支由 Lars Bak 带领的引擎团队，这支团队在 Google 内部从零写出了全新的 JavaScript 引擎 V8。V8 不像之前的引擎那样逐行解释执行 JS，而是**直接编译成机器码再执行**。结果：JS 的执行速度提升了一个数量级。之前 JS 慢是共识，之后 JS 快成了可能。
