...
- **是否有异步但没有处理竞态？** 假设这是一个带 API 调用的函数，用户连续点了 3 次排序，3 次请求都发出去了。最后一次请求最慢——3 秒后它的结果覆盖了前两次，而页面此刻显示的是第 3 次点排序的结果，但数据是第 1 次的。这个 bug 叫"竞态条件"（Race Condition）。AI 生成的异步代码几乎从来不处理它。
