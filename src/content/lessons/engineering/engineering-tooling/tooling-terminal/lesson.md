...
:::analogy
命令行就像用语音助手控制手机——说一句话（敲一行命令），它帮你完成一件事。不需要在文件夹里翻来翻去、鼠标点来点去。几个字母，一切就绪。
:::
...
这就像你输入前几个字，手机输入法就猜出你要打的整句话。
:::

:::recap
你学会了 6 个最常用的终端命令——pwd 看当前位置，ls 列出文件，cd 切换目录，mkdir 创建文件夹，echo 创建文件，code . 用 VS Code 打开。按 Tab 键还能自动补全，不用全手打。
:::
