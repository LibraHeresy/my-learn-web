...
:::analogy
Slot（插槽）就像填空题的空白横线——组件定义好"这里有一个空位"，使用者在空位上填入自己的内容。这让组件既有统一的框架，又能灵活变化内容。
:::
...
就像说明书上的标记：
- Props = 零件（数据）
- Slots = 自由发挥的部分（留给执行者填充的内容）
- Emits = 通知（告诉父组件这里发生了什么）
:::

:::task{title="动手试试 ✨"}
::::step{purpose="组件化不仅是为了复用，更是为了统一视觉风格。先定义卡片的外框（边框、圆角、阴影），为后续用 slot 填充内容做好准备。这就像先搭建一个展示柜的框架，再让不同人在里面摆放自己的展品。" expected="一个带有统一外框样式的空白卡片组件。"}
...
:::recap
你学会了用 Slot 插槽让组件接收模板内容——默认 slot 传递主体内容，具名 slot（#header、#footer）精确填充不同区域。Slot 传结构，Props 传数据，各司其职，组件既统一又灵活。
:::
