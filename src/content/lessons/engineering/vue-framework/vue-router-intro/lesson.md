...
:::analogy
Vue Router 就像一本书的目录——点击目录中的章节名，翻到对应的那一页。URL 变了，页面内容跟着变，但不需要重新加载整个页面。
:::
...
:::task{title="动手试试 ✨"}
::::step{purpose="首页是应用的入口。用路由的 path: \"/\" 映射到 Home 组件，<router-view /> 在 App.vue 中作为内容出口。路由让单页应用有了多页面的体验——URL 变了，内容变了，但页面没有白屏刷新。" expected="访问根路径 / 时显示项目列表和筛选功能。"}
首页 /：展示所有项目（列表 + 筛选）
::::

::::step{purpose="动态路由 /piece/:id 让 URL 携带参数。通过 route.params.id 获取项目 ID，根据 ID 查找对应数据并展示详情。同一个路由匹配所有项目，参数不同内容不同——这就是动态路由的核心价值。" expected="从列表点击某个项目，跳转到 /piece/xxx 并显示该项目的完整信息。"}
详情页 /piece/:id：展示单个项目的详细信息
::::
...
:::recap
你学会了用 Vue Router 实现多页面切换——定义路由规则，用 <router-link> 导航，用 <router-view> 显示匹配的页面组件。页面切换不会白屏刷新，URL 变化内容也跟着变，这就是单页应用的体验。
:::
