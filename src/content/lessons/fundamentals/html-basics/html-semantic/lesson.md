# 语义化标签 — 给页面一个清晰的结构

:::analogy
语义化标签就像给不同的房间挂门牌——厨房、卧室、书房，而不是全叫"房间1"。header、main、footer 让浏览器和搜索引擎一眼看懂你的页面结构。
:::

:::explain{title="为什么需要语义化？"}
之前我们一直用 div 来分组内容。但 div 本身没有"含义"——浏览器不知道一个 div 是导航栏还是文章正文。

语义化标签用有名字的标签来标记不同区域：header 是页头，nav 是导航菜单，main 是页面主体，section 是内容区块，article 是独立文章，footer 是页脚——看名字就知道它是什么。就像你家每个房间都有固定名字——"卧室""厨房""客厅"，而不是全部叫"房间1、房间2"。

具体写法：
- `<header>` — 页头（Logo + 导航）
- `<nav>` — 导航菜单
- `<main>` — 页面主要内容
- `<section>` — 一个内容区块
- `<article>` — 一篇独立的文章
- `<footer>` — 页脚（版权、链接）
就像完整蓝图中每个模块都有明确的名字，而不是全部标"工具1、工具2"。
:::

:::explain{title="一个典型的页面结构"}
```html
<body>
  <!-- 页头：放网站标题和导航 -->
<header>
    <h1>网站标题</h1>
    <!-- 导航：放菜单链接 -->
<nav>导航链接</nav>
  </header>
  <!-- 主体：放页面核心内容 -->
<main>
    <section>
      <h2>第一块内容</h2>
      <!-- 文章：独立的内容块 -->
<article>一篇文章</article>
    </section>
  </main>
  <!-- 页脚：放版权和联系信息 -->
<footer>
    <p>版权信息</p>
  </footer>
</body>
```
浏览器和搜索引擎看到这个结构，就像导演看到完整蓝图——一眼就知道每个部分的作用。
:::

:::example{title="看例子"}
下面的代码用语义化标签构建了一个网站页面。注意看 `<header>`、`<main>`、`<section>`、`<footer>` 是如何组织的：
```html
<!-- 页头：放网站标题和导航 -->
<header>
  <h1>古典音乐鉴赏</h1>
  <!-- 导航：放菜单链接 -->
<nav>首页 | 设计师 | 项目</nav>
</header>
<!-- 主体：放页面核心内容 -->
<main>
  <section>今日推荐</section>
  <section>本周精选</section>
</main>
<!-- 页脚：放版权和联系信息 -->
<footer>© 2026 代码阶段</footer>
```
虽然看起来和用 `<div>` 差不多，但这些标签自带"身份信息"。
:::

:::task{title="动手试试 ✨"}
::::step{purpose="练习添加语义化区块——`<section>` 表示一个**独立的内容区块**，浏览器和搜索引擎能识别它的含义。" expected="页面中多了一个名为「最新评论」的区块。注意 `<section>` 默认是块级元素，独占一行。"}
在 `<main>` 中再添加一个 `<section>`，里面写「最新评论」
::::

::::step{purpose="复习 `<a>` 标签，同时理解 `<nav>` 的语义——**nav 专门用来放导航链接**。" expected="导航栏里多了一个可点击的链接。"}
在 `<nav>` 中添加一个链接（用 `<a>` 标签）
::::

::::step{purpose="理解 `<section>` 和 `<article>` 的区别：`section` 是页面的一个区块，`article` 是可以**独立分发**的完整内容（如一篇文章、一条评论）。" expected="换成 `<article>` 后视觉上看不出区别，但这改变了元素的语义身份——搜索引擎会把它当作一篇独立文章来索引。"}
试着把某个 `<section>` 换成 `<article>`，思考：什么情况下用 article 更合适？
::::

:::

:::recap
这一节你学会了用"有名字的标签"搭建页面结构——`<header>` 是页头，`<nav>` 是导航栏，`<main>` 是主体内容，`<section>` 是内容区块，`<article>` 是独立文章，`<footer>` 是页脚。这些标签让浏览器和搜索引擎一眼就能看懂你的页面结构。现在你搭出来的网页不再是"一堆 div"，而是一个分工明确、井井有条的页面了。
:::


