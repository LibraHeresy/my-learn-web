...
:::music-analogy
语义化标签就像**总谱上的声部标注**——第一小提琴、第二小提琴、中提琴、大提琴……每个乐器组有自己的位置和身份。同样，`<!-- 页头：放网站标题和导航 -->
<header>`、`<!-- 主体：放页面核心内容 -->
<main>`、`<!-- 页脚：放版权和联系信息 -->
<footer>` 等标签告诉浏览器和搜索引擎"这是什么部分"。
:::
...
:::example{title="看例子"}
下面的代码用语义化标签构建了一个音乐网站页面。注意看 `<!-- 页头：放网站标题和导航 -->
<header>`、`<!-- 主体：放页面核心内容 -->
<main>`、`<section>`、`<!-- 页脚：放版权和联系信息 -->
<footer>` 是如何组织的：
```html
<!-- 页头：放网站标题和导航 -->
<header>
...
在 `<!-- 主体：放页面核心内容 -->
<main>` 中再添加一个 `<section>`，里面写「最新评论」
::::

::::step{purpose="复习 `<a>` 标签，同时理解 `<!-- 导航：放菜单链接 -->
<nav>` 的语义——**nav 专门用来放导航链接**。" expected="导航栏里多了一个可点击的链接。"}
在 `<!-- 导航：放菜单链接 -->
<nav>` 中添加一个链接（用 `<a>` 标签）
::::

::::step{purpose="理解 `<section>` 和 `<!-- 文章：独立的内容块 -->
<article>` 的区别：`section` 是页面的一个区块，`article` 是可以**独立分发**的完整内容（如一篇文章、一条评论）。" expected="换成 `<article>` 后视觉上看不出区别，但这改变了元素的语义身份——搜索引擎会把它当作一篇独立文章来索引。"}
试着把某个 `<section>` 换成 `<!-- 文章：独立的内容块 -->
<article>`，思考：什么情况下用 article 更合适？
::::
