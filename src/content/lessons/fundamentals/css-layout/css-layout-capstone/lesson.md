...
:::explain{title="本节地图 🗺️（约 40 分钟，六个区域一条线）"}
这一节要把前几课的技能组合成完整落地页，路线如下：

1. **先规划** — 从设计稿到 CSS，画格子再填内容
2. **导航栏** — Flexbox + Sticky
3. **英雄区** — Flexbox 居中 + 大背景
4. **特色卡片** — Flexbox 换行 vs Grid 对齐（两种都试）
5. **内容区** — Grid 两栏布局
6. **底部** — Grid 多列 + Flexbox 内排列

每个区域对应一组动手步骤，跟着步骤逐个完成即可；卡住时回到对应章节的课程复习。
:::
