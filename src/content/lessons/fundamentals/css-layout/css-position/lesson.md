...
:::explain{title="本节地图 🗺️（约 25 分钟，四个站台）"}
定位的四种模式就像四种不同的"交通方式"，逐个体验就记住了：

1. **relative** — 在自己的座位上挪一下（不脱离文档流）
2. **absolute** — 脱离队伍，站在精确坐标上（参考系是最近的已定位祖先）
3. **fixed** — 钉在屏幕上，滚动也不动（参考系是视口）
4. **sticky** — 滚到一定位置就粘住（relative + fixed 的混合）

每学完一种模式就做对应的动手步骤，再进入下一种。
:::
