...
:::recap
这一节你学会了动态创建和删除页面元素——`createElement` 凭空造出一个新元素，`appendChild` 把它加进页面，`.remove()` 让它消失。就像在演奏中即兴加入一段旋律，或者让某个声部暂时退出。配合事件绑定、输入框取值，你就能做出一个可以任意添加和删除内容的"Todo 列表"了。现在你的页面不再是固定的，可以在运行时随意增减内容。
:::
