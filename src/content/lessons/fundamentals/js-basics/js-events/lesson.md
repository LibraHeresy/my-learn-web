...
:::analogy
事件就像门铃——按一下（点击按钮），屋里的人就知道有人来了（执行代码）。没按门铃的时候，代码等着；按下去了，代码才动。
:::
...
```
事件处理函数就像一个"回响"——你按下开关（触发事件），灯亮了（执行函数）。
:::
...
:::task{title="动手试试 ✨"}
::::step{purpose="让你直观感受事件驱动编程的核心模型：用户操作（点击）→ 事件触发 → 执行回调函数 → 更新页面。就像按下开关 → 电路连通 → 灯亮了——每一步都是对前一步的响应。" expected="每点击一次按钮，计数器增加 1，页面显示「你点击了 N 次」。这是你第一次写出「能和用户互动」的代码。"}
...
修改事件回调函数中 `display.textContent` 的显示文字，换一种表达方式。比如改成 `"当前计数：第 " + count + " 次"` 或使用模板字符串自定义
::::
...
把回调函数中的 `count = count + 1` 改成 `count = count + 5`，体验每次点击跳 5 个数的加速感
::::
