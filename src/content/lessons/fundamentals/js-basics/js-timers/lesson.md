...
:::recap
这一节你学会了用定时器控制代码的执行时间——`setInterval` 每隔一段时间重复执行（像节拍器持续打拍子），`setTimeout` 等待一段时间后执行一次（像延长音记号，停一会儿再继续）。用 `clearInterval` 可以停止定时器。时间单位是毫秒（1000ms = 1 秒）。现在你的页面可以自动运行了——计数器自动涨、轮播图自动切、节拍器按时打节拍。
:::
