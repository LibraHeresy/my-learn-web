...
下面是一个更贴近实际开发的例子。`createUserCard` 函数接收用户信息，返回一段 HTML 代码。调用一次就生成一张卡片：

```js
// 定义函数：接收三个参数，返回一段 HTML 字符串
function createUserCard(name, role, email) {
  //  参数 name  → 用户姓名
  //  参数 role  → 用户职位
  //  参数 email → 用户邮箱
  ...
  html += '<div class="card">';               // 卡片容器开始
  html += '  <h2>' + name + '</h2>';           // 用传入的 name 做标题
  html += '  <p class="role">' + role + '</p>'; // 用传入的 role 做职位
  html += '  <p class="email">' + email + '</p>'; // 用传入的 email 做邮箱
  html += '</div>';                            // 卡片容器结束

  return html;  // 把拼接好的 HTML 字符串返回
}

// 调用函数：传入不同的参数，得到不同的卡片
let card1 = createUserCard("张三", "前端工程师", "zhangsan@example.com");
let card2 = createUserCard("李四", "UI 设计师", "lisi@example.com");
let card3 = createUserCard("王五", "产品经理", "wangwu@example.com");

// 把三张卡片拼在一起，显示到页面上
document.getElementById("card-container").innerHTML = card1 + card2 + card3;
```

**运行结果：** 页面上会显示三张卡片，每张卡片有不同的名字、职位、邮箱——但结构完全一样。你只写了一次 HTML 结构（函数定义里），用了三次（三次调用）。

**这个例子展示了函数的复用能力：** 如果要给每张卡片加一个"电话号码"行，你只需在函数定义里加一行 `html += '<p>电话：' + phone + '</p>';`，然后在函数签名里加一个 `phone` 参数。三张卡片都会自动多出一行。这就是"改一处，全局生效"。
:::
...
::::step{purpose="理解函数的核心机制：同一个函数，传入不同的参数，产生不同的输出。就像同一个菜谱，用鸡肉和牛肉做出的口味不同，但做法步骤完全一样。" expected="页面第一张卡片的名字、职位、邮箱变成了你输入的内容，另外两张卡片不变。函数本身一行没改。"}
1. 打开右侧编辑器的 `script.js` 文件
2. 找到第一个 `createUserCard()` 调用（大约在代码中部）
3. 把第一个参数 `"张三"` 改成你自己的名字
4. 把第二个参数 `"前端工程师"` 改成你想写的职位
5. 切换到预览区，确认第一张卡片的内容变了
::::

::::step{purpose="体验函数的复用能力：一行函数调用就生成一张完整的卡片。你不需要重新写一遍 HTML 结构，" expected="页面上出现了第四张卡片，样式和其他三张完全一样。你用一行代码创造了一张新卡片！"}
1. 在 `script.js` 中找到三张卡片拼接的那行代码（`card1 + card2 + card3`）
2. 在这三张卡片前面，用 `createUserCard()` 新创建一个 `card0` 变量
3. 把拼接改为 `card0 + card1 + card2 + card3`
4. 切换到预览区，确认第四张卡片出现
::::

::::step{purpose="练习修改函数定义：增加参数意味着函数的能力增强了。改一处定义，所有调用点都受益——这正是函数'单一真相来源'的价值。" expected="所有四张卡片都多出了一行电话信息，而你只改了函数定义和调用参数。"}
1. 打开 `script.js`
2. 给 `createUserCard` 函数增加第四个参数 `phone`
3. 在函数体内的 HTML 模板中添加一行 `<p>📞 ${phone}</p>`
4. 给每个 `createUserCard()` 调用都传入电话号码参数
5. 切换到预览区，确认所有卡片都显示了电话号码
::::
