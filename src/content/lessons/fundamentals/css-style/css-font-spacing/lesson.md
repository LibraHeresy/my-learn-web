...
:::analogy
字体选择就像穿衣服——正式场合穿正装（衬线体），日常穿便装（无衬线体）。间距就像呼吸的节奏——太挤了读不进去，太散了断断续续。
:::
...
像选衣服一样——你不会穿拖鞋去参加正式宴会。
:::

:::explain{title="间距控制"}
三个重要的间距属性：
- `letter-spacing` — 字母/汉字间距，像零件间的疏密
- `line-height` — 行高，像设计图中行与行的距离
- `text-align` — 文字对齐（`left` / `center` / `right`），像团队的队列
合理的间距让文字"透气"，就像说话时的停顿，让内容更清晰。
:::
...
:::task{title="动手试试 ✨"}
::::step{purpose="`font-family` 决定字体风格。`serif`（衬线体）典雅有装饰，`sans-serif`（无衬线体）简洁现代。像选衣服风格——不同的字体传达不同的气质。" expected="卡片文字从有装饰线的衬线体变成平滑的无衬线体。注意笔画末端的差异。"}
把 `.card` 的 `font-family` 改成 `sans-serif`，感受字体变化
::::

::::step{purpose="`line-height` 控制行与行之间的距离。值越大行越疏朗，越小行越紧密。合理的行高让文字透气，就像段落之间的空行，让文字读起来不累。" expected="`1.3` 时行间距紧凑，`2.5` 时行间距非常疏朗。`1.6~1.8` 通常是正文最舒适的阅读间距。"}
...
- 全屏布局 → `vw` / `vh`
- 容器内比例 → `%`
> 💡 类比：`px` 像尺子上的刻度（精确但死板），`rem` 像衣服的尺码（跟着基准号变），`vw` 像房间的窗户大小（跟着空间变）。
:::
