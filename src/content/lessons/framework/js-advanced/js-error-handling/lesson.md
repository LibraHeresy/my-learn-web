...
:::analogy
程序运行中难免出错——就像开车时导航信号丢了。好的错误处理不是不让错误发生，而是发生了之后系统继续运转。try/catch 就是安全气囊。
:::
...
- **类型**：数据属于哪种"种类"——数字、字符串、布尔值、对象等
- **try/catch**：JS 的异常捕获机制——try 块里放可能出错的代码，catch 块里处理错误
:::
...
- 页面白屏（一个错误导致整个脚本崩溃）
- 数据加载失败（网络问题、API 挂了）
就像一个员工突然请假，团队需要有后备方案。程序中，我们用 **try/catch** 来应对。
:::

:::example{title="try/catch — \"准备一下，看看会不会出错\""}
基本结构：
```js
try {
  // 尝试执行可能有风险的代码
...
```
`throw` 就像安检员拦下超重行李："不对，这个箱子超重了！"——主动发现并指出问题。
:::
...
:::task{title="动手试试 ✨"}
::::step{purpose="{{term:JSON.parse}} 是常见的\"炸弹\"——传入非法字符串就直接报错崩溃。{{term:try/catch}} 就是防爆服：把可能爆炸的代码放在 try 里，一旦炸了，catch 会接住它而不是让整个程序崩溃。" expected="传入合法 JSON 字符串时，返回解析后的 JS 对象——与直接调用 JSON.parse 的行为一致。"}
...
分别传入合法 JSON（{"name": "春天", "composer": "春天"}）和非法 JSON（"这不是JSON"），测试两种路径
::::
