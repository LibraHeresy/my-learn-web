...
:::analogy
数据驱动就像看仪表盘开车——仪表盘显示的是数据，你根据数据做决策。代码也一样：你只修改数据，然后调用一次渲染函数，页面自动跟上。
:::

:::prerequisite
**本节你需要知道这些词：**

- **DOM**：浏览器把网页解析成一棵"节点树"，你可以用 JS 操作它
...
:::task{title="动手试试 ✨"}
::::step{purpose="render() 是数据驱动模式的核心——它把\"数据\"翻译成\"页面\"。就像餐厅根据菜单做菜——改了菜单上的菜名（改数据），厨师重做一遍（调 render），客人吃到的就是新菜。" expected="页面初始显示空列表或预设数据，render() 调用后页面内容与 pieces 数组完全同步。"}
把数据提取成一个 pieces 数组，写一个 render() 函数：每次调用时清空容器，重新根据 pieces 生成全部 DOM
::::

::::step{purpose="数据驱动让你摆脱\"先创建 div、再设置内容、再挂载\"的命令式思维。你只需要想\"我要加一条数据\"，render 函数自动帮你把 DOM 搞定。这就是后来所有前端框架的核心思想——数据是唯一的真相来源。" expected="输入名称点击添加后，列表自动刷新展示新项目，代码中没有任何 createElement 或 innerHTML 直接操作。"}
"添加"按钮改为：pieces.push(新项目)，然后调用 render()——不再直接操作 DOM
::::
...
:::recap
你学会了数据驱动的思维方式——把数据和渲染分开，数据是"唯一的真相来源"。改数据后调用 render() 函数重新渲染页面，而不是直接操作 DOM。这就是 Vue 和 React 等前端框架的核心思想。
:::
